...
`omalhc` is a constitutional swarm mesh package at `packages/omalhc/`.
...
## Task: Constitutional Mesh (`src/omalhc/mesh.py`)
...
```python
from omalhc.mesh import ConstitutionalMesh, PeerAssignment, ValidationVote, MeshResult
...
Put errors in `src/omalhc/mesh.py` (not a separate file).
...
- Constitutional validation uses the existing `AgentDNA` from `omalhc.dna`

### Required Checks
```bash
python -m pytest packages/omalhc/tests/ -v --import-mode=importlib
...
- Update `src/omalhc/__init__.py` to export `ConstitutionalMesh, PeerAssignment, ValidationVote, MeshResult`
...
- `packages/omalhc/src/omalhc/dna.py` — AgentDNA (use this for constitutional pre-validation)
- `packages/omalhc/src/omalhc/swarm.py` — TaskDAG, SwarmExecutor (mesh integrates with this)
- `packages/omalhc/src/omalhc/artifact.py` — Artifact, ArtifactStore
- `packages/omalhc/src/omalhc/capability.py` — CapabilityRegistry patterns
- `packages/omalhc/tests/test_omalhc.py` — existing test patterns and style
