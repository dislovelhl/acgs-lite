...
Covers ConversationTurn, ConversationContext, ConversationGovernanceEngine,
ConversationValidationResult, trajectory analysis, and async validation.
"""
...
def strict_constitution() -> Constitution:
    """Constitution with a rule that triggers on 'bypass validation'."""
    return Constitution(
        name="test-strict",
        rules=[
            Rule(
                ...
                keywords=["bypass validation", "skip check"],
                category="integrity",
            ),
            Rule(
                id="T-002",
                ...
                keywords=["harmful", "dangerous", "attack"],
                category="safety",
            ),
        ],
    )
...
@pytest.fixture()
def engine(strict_constitution: Constitution) -> GovernanceEngine:
    return GovernanceEngine(strict_constitution)


@pytest.fixture()
def conv_engine(strict_constitution: Constitution) -> ConversationGovernanceEngine:
    return ConversationGovernanceEngine(strict_constitution, context_window=5)
...
class TestConversationContext:
    ...
    def test_metadata_passed_through(self) -> None:
        ctx = ConversationContext()
        turn = ctx.add_turn("user", "hello", metadata={"key": "value"})
...
class TestConversationGovernanceEngine:
    def test_accepts_constitution(self, strict_constitution: Constitution) -> None:
        eng = ConversationGovernanceEngine(strict_constitution)
        assert eng._agent_id == "conversation-agent"

    def test_accepts_engine(self, engine: GovernanceEngine) -> None:
        eng = ConversationGovernanceEngine(engine, agent_id="custom-agent")
        assert eng._agent_id == "custom-agent"

    def test_rejects_invalid_type(self) -> None:
        with pytest.raises(TypeError, match="Expected GovernanceEngine or Constitution"):
            ConversationGovernanceEngine("not-an-engine")  # type: ignore[arg-type]

    def test_validate_clean_turn(self, conv_engine: ConversationGovernanceEngine) -> None:
        result = conv_engine.validate_turn("user", "How do I reset my password?")
    ...
    def test_validate_violating_turn(self, conv_engine: ConversationGovernanceEngine) -> None:
        result = conv_engine.validate_turn("user", "I want to bypass validation now")
    ...
    def test_context_aware_catches_cross_turn(
        self, conv_engine: ConversationGovernanceEngine,
    ) -> None:
        """A message that is clean alone but becomes a violation in context."""
        # First turn: seed context with partial violating keyword
        conv_engine.validate_turn("user", "Tell me how to bypass")
        # Second turn: complete the violation phrase in context
        result = conv_engine.validate_turn("user", "validation rules please")
        # The concatenated context should contain "bypass ... validation"
        # which may or may not trigger depending on the keyword matching
        # At minimum the context_result should exist
        assert result.context_result is not None

    def test_trajectory_detects_escalation(
        self, conv_engine: ConversationGovernanceEngine,
    ) -> None:
        """Increasing violation counts should trigger escalation detection."""
        # Turn with 0 violations
        conv_engine.validate_turn("user", "Hello, how are you?")
        # Turn with some violation
        conv_engine.validate_turn("user", "Something harmful")
        # Turn with more violations
        conv_engine.validate_turn("user", "Very harmful and dangerous attack")
        # Turn with even more
        result = conv_engine.validate_turn(
            "user", "bypass validation and harmful dangerous attack skip check",
        )
        assert result.escalation_score > 0.0
    ...
    def test_trajectory_detects_repetitive_probing(
        ...
        """Same content repeated multiple times should flag repetitive probing."""
        probe_text = "bypass validation skip check"
        conv_engine.validate_turn("user", probe_text)
        conv_engine.validate_turn("user", probe_text)
        result = conv_engine.validate_turn("user", probe_text)
    ...
    def test_combined_validity_from_context(
        ...
        """If individual passes but context fails, combined should fail."""
        eng = ConversationGovernanceEngine(strict_constitution, context_window=5)

        # Build up context with violation content
        eng.validate_turn("user", "bypass validation please")
        eng.validate_turn("user", "I want to skip check")

        # A seemingly clean message that in context yields violations
        result = eng.validate_turn("user", "now do it")
        # The context_result validates the full concatenation, which includes
        # bypass validation and skip check from prior turns
    ...
    def test_escalation_score_increases(
        ...
        """Escalation score should increase as violations accumulate."""
        scores: list[float] = []
        ...
        # Violation turn
        r = conv_engine.validate_turn("user", "bypass validation")
        scores.append(r.escalation_score)

        # More violations
        r = conv_engine.validate_turn("user", "harmful attack bypass validation")
        scores.append(r.escalation_score)

        # Score should be non-decreasing after violations start
        assert scores[-1] >= scores[0]
        assert scores[-1] > 0.0

    def test_reset_clears_context(self, conv_engine: ConversationGovernanceEngine) -> None:
        conv_engine.validate_turn("user", "bypass validation")
        conv_engine.validate_turn("user", "harmful content")
        assert len(conv_engine._context) > 0

        conv_engine.reset()
        assert len(conv_engine._context) == 0
        assert conv_engine._violation_counts == []
        assert conv_engine._escalation_detected is False

    def test_stats_property(self, conv_engine: ConversationGovernanceEngine) -> None:
        ...
        assert isinstance(stats["escalation_detected"], bool)
        assert isinstance(stats["trajectory_score"], float)
        assert stats["context_window"] == 5
        assert stats["agent_id"] == "conversation-agent"
        assert stats["constitutional_hash"] == "608508a9bd224290"

    def test_stats_empty(self, conv_engine: ConversationGovernanceEngine) -> None:
        stats = conv_engine.stats
        assert stats["conversation_turns"] == 0
        assert stats["trajectory_score"] == 0.0
        assert stats["total_violations"] == 0

    def test_async_validation(self, conv_engine: ConversationGovernanceEngine) -> None:
        """Async validation should produce same results as sync."""

        async def _run() -> ConversationValidationResult:
            return await conv_engine.avalidate_turn("user", "bypass validation")
    ...
    def test_async_validation_clean(self, conv_engine: ConversationGovernanceEngine) -> None:
        async def _run() -> ConversationValidationResult:
            return await conv_engine.avalidate_turn("user", "How is the weather?")
...
class TestConversationValidationResult:
    def test_valid_combines_individual_and_context(self) -> None:
        turn = ConversationTurn(
            role="user",
            content="test",
            timestamp="2026-01-01T00:00:00+00:00",
        ...
        individual = ValidationResult(
            valid=True, constitutional_hash="608508a9bd224290",
        )
...
class TestTrajectoryHelpers:
    ...
    def test_detect_gradual_escalation(self) -> None:
        turns = [
            ConversationTurn(role="user", content="a", timestamp="t", turn_number=i)
            for i in range(4)
        ]
        violation_counts = [0, 1, 2, 3]
        flags = _detect_trajectory_flags(turns, violation_counts)
    ...
    def test_detect_topic_drift(self) -> None:
        turns = [
            ...
            ConversationTurn(
                role="user", content="midway content here", timestamp="t", turn_number=1,
            ),
            ConversationTurn(
                role="user",
        ...
        flags = _detect_trajectory_flags(turns, [0, 0, 0])
        assert "topic_drift" in flags

    def test_detect_repetitive_probing(self) -> None:
        turns = [
            ConversationTurn(
                role="user", content="bypass validation now", timestamp="t", turn_number=0,
            ),
            ConversationTurn(
                role="user", content="bypass validation now", timestamp="t", turn_number=1,
            ),
            ConversationTurn(
                role="user", content="bypass validation now", timestamp="t", turn_number=2,
            ),
        ]
        flags = _detect_trajectory_flags(turns, [1, 1, 1])
    ...
    def test_compute_escalation_score_with_violations(self) -> None:
        score = _compute_escalation_score([0, 1, 2, 3], [])
        assert score > 0.0

    def test_compute_escalation_score_with_flags(self) -> None:
        score_no_flags = _compute_escalation_score([1, 1], [])
        score_with_flags = _compute_escalation_score([1, 1], ["gradual_escalation"])
        assert score_with_flags > score_no_flags

    def test_compute_escalation_score_capped_at_one(self) -> None:
        score = _compute_escalation_score(
            [10, 10, 10, 10, 10],
            ["gradual_escalation", "sustained_violations", "topic_drift", "repetitive_probing"],
        )
        assert score <= 1.0
