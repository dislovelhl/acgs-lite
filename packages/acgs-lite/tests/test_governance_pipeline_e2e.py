...
class TestGovernancePipelineE2E:
    ...
    def test_case_timeout_and_requeue_pipeline(self) -> None:
        ...
        expired = case_mgr.expire_stale(_now=_ts(35))  # past submission deadline
    ...
    def test_maci_constraints_enforced_through_pipeline(self) -> None:
        ...
        pool.register("miner-1", trust_score=0.95, domains=["finance"], model="gpt-4")
        selector = ValidatorSelector(pool)
        case_mgr = CaseManager()
    ...
    def test_validator_pool_diversity_affects_selection(self) -> None:
        ...
        for i in range(6):
            mono_pool.register(f"v{i}", trust_score=0.9, domains=["fin"], model="gpt-4")
        ...
        models = ["gpt-4", "claude-3", "gemini-2", "llama-3", "mistral-2", "qwen-2"]
