...
class TestComputeEvalReport:
    ...
    def test_basic_computation(self):
        ...
        report = TestReport(
            suite_name="test",
            results=[
                self._make_result("c1", TestOutcome.PASS, "deny", ["R1"]),
                self._make_result("c2", TestOutcome.PASS, "allow", []),
                self._make_result("c3", TestOutcome.FAIL, "allow", []),  # R1 should have fired
            ],
        )
    ...
    def test_empty_report(self):
        report = TestReport(suite_name="empty", results=[])
        eval_report = compute_eval_report(
            report,
            all_rule_ids=["R1"],
            expected_rules_map={},
