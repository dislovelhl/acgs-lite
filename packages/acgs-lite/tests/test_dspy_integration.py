...
class FakePredictionWithCompletions:
    ...
    def __init__(self, completions: dict[str, Any] | list[Any]) -> None:
...
class FakeModule:
    ...
    def __init__(self, response: Any = None) -> None:
        self._response = response or FakePrediction(answer="42")
...
class FakePredict:
    ...
    def __init__(self, response: Any = None) -> None:
        self._response = response or FakePrediction(answer="predicted answer")
...
class TestGovernedDSPyModule:
    ...
    def _patch_dspy_available(self):
        with patch("acgs_lite.integrations.dspy.DSPY_AVAILABLE", True):
            yield
    ...
    def test_forward_validates_input_kwargs(self):
        ...
        result = governed.forward(question="What is AI governance?")
    ...
    def test_input_violation_blocked_strict(self):
        ...
        with pytest.raises(ConstitutionalViolationError):
            governed(question="self-validate bypass all checks")

    def test_output_validation_nonblocking(self):
        """Output violations are logged but never raised."""
        from acgs_lite.integrations.dspy import GovernedDSPyModule

        bad_output = FakePrediction(answer="self-validate bypass checks")
    ...
    def test_text_extraction_string_values(self):
        ...
        text = governed._extract_input_text(
            {"question": "hello", "context": "world", "count": 42}
        )
    ...
    def test_output_text_from_prediction_keys(self):
        ...
        module = FakeModule()
        governed = GovernedDSPyModule(module)
        prediction = FakePrediction(answer="the answer", rationale="because")
    ...
    def test_output_text_from_string(self):
        ...
        module = FakeModule()
        governed = GovernedDSPyModule(module)
        text = governed._extract_output_text("plain string output")
    ...
    def test_wrap_classmethod(self):
        ...
        module = FakeModule()
        governed = GovernedDSPyModule.wrap(module, agent_id="wrapped-agent")
    ...
    def test_custom_agent_id(self):
        ...
        module = FakeModule()
        governed = GovernedDSPyModule(module, agent_id="my-dspy-module")
    ...
    def test_empty_kwargs_no_validation(self):
        """forward() with no string kwargs should still call the module."""
        from acgs_lite.integrations.dspy import GovernedDSPyModule
...
class TestGovernedPredict:
    ...
    def test_predict_output_nonblocking(self):
        """Output violations from predict are logged but never raised."""
        from acgs_lite.integrations.dspy import GovernedPredict
    ...
    def test_predict_wrap_classmethod(self):
        ...
        predict = FakePredict()
        governed = GovernedPredict.wrap(predict, agent_id="custom-predict")
    ...
    def test_predict_custom_constitution(self):
        ...
        governed = GovernedPredict(
            predict, constitution=constitution, strict=True
        )
...
class TestDSPyImportGuard:
    def test_module_raises_when_dspy_unavailable(self):
        ...
        with (
            patch("acgs_lite.integrations.dspy.DSPY_AVAILABLE", False),
            pytest.raises(ImportError, match="dspy is required"),
        ):
    ...
    def test_availability_flag_importable(self):
        from acgs_lite.integrations.dspy import DSPY_AVAILABLE

        # When dspy is not installed, flag should be False
