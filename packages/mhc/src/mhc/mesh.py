...
def _compute_merkle_root(
    ...
    if not vote_hashes:
        votes_root = hashlib.sha256(b"empty").hexdigest()[:16]
    else:
        votes_root = vote_hashes[0]
        for vh in vote_hashes[1:]:
            combined = f"{votes_root}:{vh}"
            votes_root = hashlib.sha256(combined.encode()).hexdigest()[:16]

    # Root: combine leaf and votes
    root = hashlib.sha256(f"{leaf}:{votes_root}".encode()).hexdigest()[:16]
    return root
