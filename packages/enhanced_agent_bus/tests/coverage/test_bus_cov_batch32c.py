...
class TestOCIRegistryClientMisc:
    ...
    def test_get_stats(self):
        client = OCIRegistryClient("https://registry.example.com", registry_type=RegistryType.HARBOR)
...
class TestListAmendmentsEndpoint:
    ...
    async def test_list_amendments_success_with_status_filter(self):
        ...
        with patch(
            ...
            result = await list_amendments(
                status="proposed", proposer_agent_id=None,
                limit=10, offset=0, order_by="created_at", order="asc",
            )
    ...
    async def test_list_amendments_no_filter(self):
        ...
        with patch(
            ...
            result = await list_amendments(
                status=None, proposer_agent_id="agent-1",
                limit=50, offset=0, order_by="created_at", order="desc",
            )
        ...
        with patch(
            ...
            with pytest.raises(Exception) as exc_info:
                await list_amendments(
                    status=None, proposer_agent_id=None,
                    limit=50, offset=0, order_by="created_at", order="desc",
                )
    ...
    async def test_approve_success_fully_approved(self):
        ...
        with patch(
            ...
            with patch(
                ...
                assert "fully approved" in result.next_steps[0].lower() or "approved" in result.message.lower()
...
class TestRejectAmendmentEndpoint:
    ...
    async def test_reject_uses_header_agent_id(self):
        ...
        with patch(
            ...
            assert call_kwargs.kwargs.get("agent_id") == "header-agent" or call_kwargs[1].get("agent_id") == "header-agent"
...
class TestRollbackEndpoint:
    ...
    async def test_rollback_success(self):
        ...
        with patch(
            ...
            "enhanced_agent_bus.constitutional.review_api.GovernanceMetricsCollector",
            return_value=mock_metrics,
        ), patch(
            "enhanced_agent_bus.constitutional.review_api.DegradationDetector",
            return_value=mock_degradation,
        ), patch(
            "enhanced_agent_bus.constitutional.review_api.rollback_amendment",
