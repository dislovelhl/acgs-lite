...
class TestMessageRouterProtocol:
    """Tests for MessageRouter Protocol definition."""
    ...
    def test_non_conforming_fails_isinstance(self):
        assert not isinstance(object(), MessageRouter)


class TestConcreteMessageRouter:
    """Tests exercising concrete MessageRouter logic."""

    async def test_route_returns_to_agent(self, agent_message):
        router = ConcreteMessageRouter()
        reg = ConcreteAgentRegistry()
        target = await router.route(agent_message, reg)
        assert target == "agent-b"

    async def test_route_no_to_agent_returns_none(self):
        router = ConcreteMessageRouter()
        reg = ConcreteAgentRegistry()
        msg = AgentMessage(content={}, from_agent="a", to_agent="")
        target = await router.route(msg, reg)
        assert target is None

    async def test_broadcast_all_agents(self):
        router = ConcreteMessageRouter()
        reg = ConcreteAgentRegistry()
        await reg.register("agent-1")
        await reg.register("agent-2")
        msg = AgentMessage(content={}, from_agent="a", to_agent="")
        targets = await router.broadcast(msg, reg)
    ...
    async def test_broadcast_with_exclude(self):
        ...
        await reg.register("agent-1")
        await reg.register("agent-2")
        await reg.register("agent-3")
        msg = AgentMessage(content={}, from_agent="a", to_agent="")
        targets = await router.broadcast(msg, reg, exclude=["agent-2"])
        assert "agent-1" in targets
