...
class TransformerScorer:
    """ML-based impact scorer using Hugging Face sentiment analysis.

    By default this follows the current Transformers pipeline default for
    ``sentiment-analysis`` instead of pinning an older local model ID.
    ...
    def classifier(self) -> Any:
        if self._classifier is None:
            ...
            self._classifier = classifier_factory(task, **pipeline_kwargs)
    ...
    def score(self, content: str) -> float:
        ...
        try:
            ...
            base_score = raw_score if result["label"] == "NEGATIVE" else 1 - raw_score
            rule_score = RuleBasedScorer().score(content)
            return float(min(0.6 * base_score + 0.4 * rule_score, 1.0))
...
class ConstitutionalImpactScorer:
    ...
    def _get_context_modifier(self, context: dict[str, Any]) -> float:
        ...
        if context.get("authenticated"):
            modifier *= 0.9
        raw_success_rate = context.get("success_rate", 0.5)
        success_rate = (
            float(raw_success_rate) if isinstance(raw_success_rate, (int, float)) else 0.5
        )
...
def score_impact(content: str, **kwargs: Any) -> float:
    ...
    return cast(float, ConstitutionalImpactScorer().score(content, **kwargs)["score"])
