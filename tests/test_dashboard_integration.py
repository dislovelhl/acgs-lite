...
class TestASGIApp:
    ...
    @pytest.mark.asyncio
    async def test_proposals_endpoint_defaults_empty(self, dashboard):
        app = create_dashboard_asgi_app(dashboard)
        msgs = await self._call(app, "/api/proposals")
        assert msgs[0]["status"] == 200
        data = json.loads(msgs[1]["body"])
        assert data == {"proposals": [], "count": 0}

    @pytest.mark.asyncio
    async def test_proposals_endpoint_uses_injected_provider(self, dashboard):
        def proposal_provider() -> list[dict[str, object]]:
            return [{"proposal_id": "p-1", "status": "queued"}]

        app = create_dashboard_asgi_app(dashboard, proposal_provider=proposal_provider)
        msgs = await self._call(app, "/api/proposals")
        assert msgs[0]["status"] == 200
        data = json.loads(msgs[1]["body"])
        assert data["count"] == 1
        assert data["proposals"] == [{"proposal_id": "p-1", "status": "queued"}]

    @pytest.mark.asyncio
    async def test_proposals_endpoint_fails_closed_when_provider_raises(self, dashboard):
        def proposal_provider() -> list[dict[str, object]]:
    ...
    @pytest.mark.asyncio
    async def test_proposals_endpoint_accepts_iterable_provider(self, dashboard):
        def proposal_provider():
            yield {"proposal_id": "p-1", "status": "queued"}
            yield {"proposal_id": "p-2", "status": "approved"}
        ...
        data = json.loads(msgs[1]["body"])
        assert data["count"] == 2
        assert data["proposals"][1] == {"proposal_id": "p-2", "status": "approved"}
